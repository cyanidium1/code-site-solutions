...
- Remaining: apply §7 palette in dashboard (buttons still default blue); re-click "Check now" (GCM status) to confirm the error cleared; after deploy, confirm choice persists on production HTTPS (the `secure` consent cookie can't persist on local HTTP).
