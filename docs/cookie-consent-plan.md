...
- **Closed 02.07.2026:** live on https://www.code-site.art/, dashboard "Check GCM status" reports **No error detected**, banner Active. Palette customization turned out to be unavailable on the free plan (contrary to earlier research) — default blue buttons accepted for now. If the look becomes a problem, either the Basic plan (~$10/mo, custom CSS) or the custom banner (§4 Option A) are the upgrade paths.
