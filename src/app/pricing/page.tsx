...
const PRICING_FAQ = [
  {
    ...
    a: "Бо це ваш час. Ви заходите на сайт, бачите вилку — розумієте, чи проект у вашому бюджеті. Не доводиться писати, чекати менеджера, дзвонити. Ми так само поважаємо ваш час, як і свій. <em>Точна цифра</em> — після 30-хв брифу.",
  },
  {
    q: "Чому індустріальні сайти (Industry Pro) дорожчі за Business?",
    a: "Бо включають специфічні інтеграції (<em>Helsi, Clio, MEDoc</em>) які потребують технічного дослідження і реалізації. Плюс compliance під галузь (МОЗ для медицини, RODO для юристів, GDPR для бухгалтерії). Це окремий рівень роботи. Деталі на сторінках /sites-for/medicine, /sites-for/legal, /sites-for/accounting.",
  },
  {
    q: "Чи можу домовитися про знижку?",
    a: "Так, у двох випадках: <em>знижка 10%</em> при оплаті 100% наперед (одним платежем). Або проект-партнерство (рекомендація на ваш сайт у нашому портфоліо + open-source-кейс) — обговорюємо індивідуально.",
  },
  {
    q: "Що якщо мій проект не вписується в жоден тир?",
    a: "Розповідайте — обговоримо. Можливі варіанти: 1) трохи більше обсягу за фіксовану ціну сусіднього тиру; 2) <em>кастомний quote</em> (для проектів складніше Enterprise); 3) розбити на 2 етапи (MVP + розширення). Не змушуємо вас вписуватись у наші коробки.",
  },
  {
    q: "Чи можна оплатити криптою?",
    a: "Так. <em>USDT TRC20</em> — стандартно. Інші стейблкоїни (USDC) — обговоримо. Bitcoin / Ethereum — теж можна, але з фіксацією курсу на момент платежу. Для іноземних клієнтів крипта — найпростіший спосіб.",
  },
  {
    q: "Чи є розстрочка для великих проектів?",
    a: "Для проектів від $10 000 — так. Стандартна схема: <em>30% старт + 35% після затвердження дизайну + 35% після здачі</em>. Для Enterprise (від $14 000) — можемо розбити на 4 платежі під milestones. Підписання договору — обов'язкове.",
  },
  {
    q: "Як рахуються інтеграції?",
    a: "Кожна стороння інтеграція (CRM, email, payment, calendar, e-sign) — <em>від $200 до $500</em> залежно від складності. Складні бекофісні інтеграції (1С, BAS, кастомні API) — окремий quote, $1 000-3 000. Точна цифра — після технічного аудиту.",
  },
  {
    q: "Що якщо потрібні зміни після запуску?",
    a: "1 рік — гарантія: виправлення багів, дрібні правки безкоштовно. Нові фічі — <em>$40/год</em> (за погодинною ставкою) або monthly retainer $200-500/міс залежно від обсягу. Великий редизайн — окремий проект. Кажемо одразу, скільки буде коштувати.",
  },
];

/* ─── JSON-LD ────────────────────────────────────────────────────────────── */

const ORG_ID = "https://code-site.art/#organization";
  ...
  return (
    <>
      ...
      <PageHero
        ...
        sub="Без «під запит». Без прихованих платежів. Платите фіксовану ціну за фіксований обсяг роботи."
      />
