...
import { MED_STOMATOLOHIIA_PROSE_UK } from "@/content/uk/medicine-children-prose";
...
const TITLE = `Сайт для стоматології — ${M.price} за ${M.term}`;
const DESCRIPTION =
  `Розробка сайту стоматології під ключ: онлайн-запис, лікарі і ціни, інтеграція з CRM. ${M.price}, запуск за ${M.term}. Прорахунок за 24 години.`;
...
const STOM_FAQ: { q: string; a: RichText }[] = [
  {
    ...
    a: [
      "Сайт стоматології під ключ — ",
      { em: M.price },
      `, фікс-ціна в договорі: онлайн-запис, каталог лікарів, прайс, запуск за ${M.term}. Мережа з кількома філіями і ДМС-інтеграцією — окремий проєкт, ${M.custom}. Точну ціну з додатками дає `,
  ...
  },
  {
    ...
    a: [
      `Робимо компактніший формат: особиста сторінка лікаря з послугами, кейсами «до/після», відгуками і тим самим онлайн-записом — ${M.practice} за ${M.practiceTerm}. Це краще за профіль в агрегаторі: пацієнт записується до вас, а не до сусіда в списку.`,
    ],
  },
  {
  ...
  return (
    <>
      ...
      <PageHero
        ...
        headline={
          <>
            Створення сайту для стоматології — <em>{M.price} за {M.term}</em>
          </>
        }
        sub={
          <>
      ...
      <Bento
        ...
        cells={[
          ...
          },
          {
            ...
            body: `Каталог послуг і цін, сторінки лікарів, онлайн-запис із CRM, локальне SEO. Запуск за ${M.term}.`,
            stat: M.price,
            span: "2x1",
          },
          {
      ...
      <section className="bg-bg px-6 sm:px-8 lg:px-12 py-14">
        <div className="max-w-container mx-auto">
          <h2 className="m-0 font-actay uppercase font-bold text-[clamp(22px,2.6vw,34px)] leading-[1.15] text-ink">
            Як проходить розробка сайту стоматології
          </h2>
          <div className="mt-6 max-w-[760px] font-sans text-[15.5px] leading-[1.7] text-ink-dim space-y-4">
            <p className="m-0">
              Дні 1–3 — структура. Ми розбираємо ваші послуги на
              сторінки так, як їх шукають пацієнти: не «терапевтична
              стоматологія», а «лікування карієсу», «видалення зуба мудрості»,
              «брекети для підлітків». Кожна така сторінка — окрема точка входу
              з Google, зі своєю ціною, фото робіт і кнопкою запису. Паралельно
              збираємо прайс у форматі, який не лякає: вилка цін і пояснення,
              від чого залежить фінальна сума.
            </p>
            <p className="m-0">
              Дні 4–10 — дизайн і тексти. Стоматологія — ніша, де
              пацієнт приходить із тривогою, тому дизайн працює на спокій:
              реальні фото кабінетів замість стоку, лікарі з іменами і стажем,
              чесні відповіді на «чи буде боляче» і «скільки це коштує». Тексти
              пишемо ми, ваш лікар лише вичитує медичну частину — на це
              потрібна година, не день.
            </p>
            <p className="m-0">
              Дні 11–21 — код, інтеграції і запуск: підключаємо
              онлайн-запис до вашої CRM, налаштовуємо SMS-підтвердження,
              переносимо домен без втрати позицій і передаємо доступи. Далі
              рік гарантійної підтримки — він уже в ціні. Побачити, як це
              виглядає в живому проєкті, можна в кейсі клініки Efedra в
              портфоліо.
            </p>
          </div>
        </div>
      ...
      <ProseSections items={MED_STOMATOLOHIIA_PROSE_UK} />
