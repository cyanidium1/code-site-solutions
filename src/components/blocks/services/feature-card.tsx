...
export function SecondaryFeatureCard({ icon, title, items }: Feature) {
  ...
  return (
    <div className="relative px-4 py-3.5 border border-line rounded-card bg-[oklch(0.16_0.005_300)] transition-[border-color,background] duration-[200ms] flex items-center gap-3 hover:border-line-strong hover:bg-[oklch(0.18_0.005_300)] md:px-5 md:py-4 md:gap-4">
      <div className="w-8 h-8 shrink-0 rounded-ctl bg-accent-12 text-accent-soft border border-accent-22 flex items-center justify-center md:w-9 md:h-9 [&>svg]:w-[18px] [&>svg]:h-[18px]">
        {icon}
      </div>
      <div className="flex-1 min-w-0">
        ...
        {blurb ? (
          <div className="text-[12px] leading-[1.45] text-ink-dim line-clamp-1 [&_em]:not-italic [&_em]:text-ink [&_em]:font-medium md:text-[12.5px]">
            {blurb}
          </div>
        ) : null}
      </div>
...
export function FeatureCard({ icon, title, items, bg }: Feature) {
  return (
    <div className="relative pt-[22px] px-5 pb-6 border border-line rounded-card bg-[oklch(0.16_0.005_300)] flex flex-col gap-3.5 overflow-hidden isolate md:pt-7 md:px-[26px] md:pb-[30px] md:gap-[18px] after:content-[''] after:absolute after:inset-0 after:-z-[1] after:pointer-events-none after:transition-opacity after:duration-300 csb-services-card-wash">
      ...
      />
      <div className="w-[38px] h-[38px] rounded-ctl bg-brand-gradient flex items-center justify-center text-[oklch(1_0_0_/_0.95)] shadow-[0_8px_20px_oklch(from_var(--color-accent)_l_c_h_/_0.3)] md:w-11 md:h-11 md:rounded-ctl">
        {icon}
      </div>
      ...
      <PhoneMore>
      <ul className="pm-cap-2 flex flex-col gap-2.5 [&>li]:flex [&>li]:items-start [&>li]:gap-2.5 [&>li]:text-[12px] [&>li]:leading-[1.5] [&>li]:text-ink-dim [&>li_em]:not-italic [&>li_em]:text-ink [&>li_em]:font-medium md:[&>li]:text-[13px]">
        {items.map((it, i) => (
          <li key={i}>
